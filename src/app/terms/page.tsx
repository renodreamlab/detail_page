...
export default function TermsPage() {
  return (
    <LegalPage title="이용약관" updated="2026-07-29">
      ...
      <section>
        <h2>제4조 (크레딧, API 키와 생성 비용)</h2>
        <ul>
          <li>일반 계정은 서비스가 제공하는 서버 API 키로 생성하며, 성공한 결과물을 기준으로 크레딧이 차감됩니다. 크레딧은 결제 페이지에서 충전할 수 있습니다.</li>
          <li>크레딧의 차감 기준과 환불 조건은 <a href="/refund">환불 정책</a>을 따릅니다.</li>
          <li>수강생 계정 등 본인 API 키(OpenAI, Google, fal 등)를 사용하는 경우 해당 외부 API 사용 요금은 이용자 본인이 부담합니다.</li>
          <li>본인 API 키는 이용자의 브라우저에만 보관되고 생성 요청 처리 시에만 전송되며, 운영자는 이를 서버에 저장하지 않습니다.</li>
        </ul>
      </section>
      <section>
        <h2>제5조 (데이터 저장 정책)</h2>
